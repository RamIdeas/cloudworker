...
describe('cloudworker-e2e', async () => {
  ...
  test('simple 200', async (cb) => {
    ...
    `
    server = new Cloudworker(script).listen(8080)
  ...
  test('cache', async (cb) => {
    ...
    server = new Cloudworker(script, {enableCache: true}).listen(8080)
  ...
  test('wasm', async (cb) => {
    ...
    server = new Cloudworker(script, {bindings: bindings}).listen(8080)
    const res = await axios.get('http://localhost:8080', defaultAxiosOpts)
    expect(res.status).toEqual(200)
  ...
  test.skip('test cloudflare generate and verify recipes', async (cb) => {
    ...
    server = new Cloudworker(cf_generate_script).listen(8080)
